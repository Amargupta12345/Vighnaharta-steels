...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { name, description = '', icon = '📦', color = 'from-blue-50 to-blue-100', order = 0, active = true } = body;
    ...
    const category = await Category.create({ name: name.trim(), slug, description, icon, color, order, active });
